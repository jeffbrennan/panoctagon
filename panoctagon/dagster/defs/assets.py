...
@dbt_assets(manifest=panoctagon_project.manifest_path, pool=POOL)
def panoctagon_dbt_assets(context: AssetExecutionContext, dbt: DbtCliResource) -> Any:
    yield from dbt.cli(["build"], context=context).stream()


@asset(compute_kind="python", key=["ufc_events"], deps=["divisions"], pool=POOL)
...
@asset(compute_kind="python", key=["scrape_fights"], deps=["ufc_events"], pool=POOL)
...
@asset(compute_kind="python", key=["ufc_fighters_stg"], deps=["scrape_fighters"], pool=POOL)
...
@asset(compute_kind="python", key=["ufc_fighters"], deps=["scrape_fighter_bio"], pool=POOL)
...
@asset(compute_kind="python", key=["bfo_raw_odds"], deps=["scrape_bfo_fighters"], pool=POOL)
def dagster_scrape_bfo_odds(context: AssetExecutionContext) -> None:
    n_downloaded = scrape.fight_odds()
    context.add_output_metadata({"n_records": n_downloaded})


@asset(compute_kind="python", key=["bfo_fighter_odds"], deps=["scrape_bfo_fighters"], pool=POOL)
...
@asset(compute_kind="python", key=["divisions"], deps=["promotions"], pool=POOL)
